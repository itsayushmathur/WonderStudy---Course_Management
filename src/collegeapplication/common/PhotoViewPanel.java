	...
	public PhotoViewPanel(FacultyPanel facultyPanel, int maxphoto) {
		...
		for (int i = 0; i < row; i++) {
			...
			for (int j = 0; j < totalcolumn; j++) {
				...
				panel[i][j].addMouseListener(new MouseAdapter() {
					...
					public void mousePressed(MouseEvent e) {
						if (e.getButton() == MouseEvent.BUTTON1) {
							...
							Faculty f = new FacultyData()
									.getFacultyInfobyId(Integer.parseInt(e.getComponent().getName()));
							if (fp.am != null) {
				...
				profilepiclabel[i][j].setIcon(new ImageIcon(
						f.getProfilePic(profilepiclabel[i][j].getWidth() + ((maxphotosinrow * 10) / 4 + 1),
								profilepiclabel[i][j].getHeight())));
	...
	public PhotoViewPanel(StudentPanel sp, int maxphoto) {
		...
		for (int i = 0; i < row; i++) {
			...
			for (int j = 0; j < totalcolumn; j++) {
					...
					public void mousePressed(MouseEvent e) {
						if (e.getButton() == MouseEvent.BUTTON1) {
							Student s = new StudentData()
									.getStudentDetails(Integer.parseInt(e.getComponent().getName()));
				...
				profilepiclabel[i][j].setText("image");
				profilepiclabel[i][j].setIcon(new ImageIcon(
						s.getProfilePic(profilepiclabel[i][j].getWidth() + ((maxphotosinrow * 10) / 4 + 1),
								profilepiclabel[i][j].getHeight())));
				...
				degreelabel[i][j] = new JLabel();
				degreelabel[i][j].setVerticalAlignment(SwingConstants.TOP);
				degreelabel[i][j].setText(new CourseData().getsemoryear(s.getCourseCode()) + "-" + s.getSemorYear()
						+ " " + " (" + s.getCourseCode() + ")");
				degreelabel[i][j].setHorizontalAlignment(SwingConstants.CENTER);
				degreelabel[i][j].setFont(new Font("Tahoma", Font.PLAIN, changeDegreeFont()));
				degreelabel[i][j].setBounds(0, profilepiclabel[i][j].getHeight() + 25, panel[i][j].getWidth(), 22);
