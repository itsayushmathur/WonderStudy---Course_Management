...
public class NotificationPanel extends JPanel {
	...
	public NotificationPanel(StudentMain sm) {
		this();
		this.sm = sm;
		addNotification(sm.s.getCourseCode(), sm.s.getSemorYear(), "Student", sm.s.getUserId(),
				sm.s.getAdmissionDate());
	}

	public NotificationPanel(FacultyMain fm) {
		this();
		this.fm = fm;
		addNotification(fm.f.getCourseCode(), fm.f.getSemorYear(), "Faculty", fm.f.getFacultyId() + "",
				fm.f.getJoinedDate());

	}

	public void addNotification(String coursecode, int semoryear, String userprofile, String userid,
			String joinedtime) {
		ArrayList<Notification> list = new NotificationData().getNotifications(coursecode, semoryear, userprofile,
				joinedtime);
				...
				if (e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() > 0) {
					...
					if (title.equals("New Student")) {
						...
						if (sm != null) {
							...
							sm.marksheetpanelscroll = new JScrollPane(marksheetpanel,
						...
						if (sm != null) {
							sm.notificationpanel.setVisible(false);
							sm.attendancereportpanel = new AttendanceReportPanel(sm, sm.notificationpanel);
							sm.attendancereportpanel.setVisible(true);
							sm.attendancereportpanelscroll = new JScrollPane(sm.attendancereportpanel,
									JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
							sm.attendancereportpanelscroll.setBounds(sm.panelx, sm.panely, 1116, 705);
							sm.attendancereportpanelscroll.setVisible(true);
							sm.attendancereportpanelscroll.setName("Attadance Report Panel Scroll");
							sm.attendancereportpanelscroll.getVerticalScrollBar().setUnitIncrement(16);
							sm.contentPane.add(sm.attendancereportpanelscroll);
							for (Component c : sm.attendancereportpanelscroll.getComponents()) {
								c.setBackground(Color.white);
							}
						}
