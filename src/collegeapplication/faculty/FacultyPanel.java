...
public class FacultyPanel extends JPanel implements ActionListener {
	...
	public FacultyPanel(FacultyMain fm) {
		...
		this.addnewfaculty.setVisible(false);
		this.viewbutton.setLocation(addnewfaculty.getX(), addnewfaculty.getY());
		condition = " where coursecode='" + fm.f.getCourseCode() + "' and semoryear=" + fm.f.getSemorYear()
				+ " and facultyid!=" + fm.f.getFacultyId();
	...
	private FacultyPanel() {
		...
		addnewfaculty = new JButton("Add Faculty");
		addnewfaculty.setBorder(new EmptyBorder(0, 0, 0, 0));
		addnewfaculty.setBounds(932, 139, 153, 33);
		panel.add(addnewfaculty);
		addnewfaculty.setFocusable(false);
		addnewfaculty.setForeground(Color.BLACK);
		addnewfaculty.setFont(new Font("Segoe UI", Font.BOLD, 15));
		addnewfaculty.setCursor(new Cursor(Cursor.HAND_CURSOR));

		addnewfaculty.setBackground(new Color(255, 255, 255));
	...
	public void createphotoviewpanel() {
		...
		this.photoviewscrollpane = new JScrollPane(photoviewpanel, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
