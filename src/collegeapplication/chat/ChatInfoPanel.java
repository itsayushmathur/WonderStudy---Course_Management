...
public class ChatInfoPanel extends JPanel {
	...
	public ChatInfoPanel() {
		...
		contactnumberlabel = new JLabel("9889245681");
		...
		emailidlabel = new JLabel("mathurayush121@gmail.com");
		...
		memberspanelscroll = new JScrollPane(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	public void setData(Student s) {
		...
		profilepic.setIcon(new ImageIcon(
				s.getRoundedProfilePic(profilepic.getWidth(), profilepic.getHeight(), profilepic.getWidth())));
	...
	public void setData(Faculty f) {
		...
		profilepic.setText("");
		profilepic.setIcon(new ImageIcon(
				f.getRoundedProfilePic(profilepic.getWidth(), profilepic.getHeight(), profilepic.getWidth())));
		bottomlabel.setText(
				"(" + f.getFacultyId() + ") " + f.getCourseCode() + "-" + f.getSemorYear() + "-" + f.getSubject());
	...
	public void setData(Group g) {
		...
		BufferedImage image = ImageUtil.toBufferedImage(
				g.getImage().getScaledInstance(profilepic.getWidth(), profilepic.getHeight(), Image.SCALE_SMOOTH));
	...
	public void setData(Admin a) {
		...
		profilepic.setText("");
		profilepic.setIcon(new ImageIcon(
				a.getRoundedProfilePic(profilepic.getWidth(), profilepic.getHeight(), profilepic.getWidth())));
	...
	private void membersPanel(Group g) {
		...
		if (admin != null) {
			...
			JPanel facultypanel = createPanel(faculty.getProfilePic(), "You", "Faculty-" + faculty.getFacultyId(),
					true);
		...
		if (gn.contains("Official")) {
			if (admin == null) {
				JPanel adminpanel = createPanel(g.getImage(), "Principal", "Admin",
						new AdminData().getAdminData().getActiveStatus());
				memberspanel.add(adminpanel);
				total++;
			}
		...
		for (ContactInfo c : ContactListPanel.contactinfo) {
			...
			if ((gn.contains("Students") || gn.contains("Official")) && c.getClassName().equals("Student")) {
				...
				if (s.getCourseCode().equals(g.getCourseCode()) && s.getSemorYear() == g.getSemorYear()) {
					JPanel panel = createPanel(s.getProfilePic(), s.getFullName(), "Student-" + s.getUserId(),
							s.getActiveStatus());
				...
				if (f.getCourseCode().equals(g.getCourseCode()) && f.getSemorYear() == g.getSemorYear()) {
					JPanel panel = createPanel(f.getProfilePic(), f.getFacultyName(), "Faculty-" + f.getFacultyId(),
							f.getActiveStatus());
					memberspanel.add(panel);
					total++;
				}
...
//		memberspanel.revalidate();
	...
	public JPanel createPanel(Image image, String username, String lastlogin, boolean isactive) {
		...
		} else if (lastlogin.contains("Members") || lastlogin.equals("Admin") || lastlogin.contains("Faculty")
				|| lastlogin.contains("Student")) {
