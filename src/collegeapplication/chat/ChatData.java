...
public class ChatData {
	...
	public ArrayList<ChatUser> getUserMessages(ChatUser u) {
		...
		try {
			...
			if (u.getToUserId().contains("Group")) {
				...
				query = "select * from chat where (touserid='" + u.getToUserId() + "' and fromuserid='"
						+ u.getFromUserId() + "') or (fromuserid='" + u.getToUserId() + "' and touserid='"
						+ u.getFromUserId() + "')";
			}
			Statement pr = con.createStatement();
			ResultSet rs = pr.executeQuery(query);
	...
	public NewMessage getNewMessages(String fromuserid, String touserid) {
		...
		try {
			...
			if (touserid.contains("Group")) {
				query = "select readby,message,fromuserid,messagetime,messagedate from chat where (touserid='"
						+ touserid + "' and fromuserid!='" + fromuserid + "')";

			} else {
				query = "select readby,message,fromuserid,messagetime,messagedate from chat where (touserid='"
						+ fromuserid + "' and fromuserid='" + touserid + "')";
			}
			Statement pr = con.createStatement();
			ResultSet rs = pr.executeQuery(query);
			while (rs.next()) {
	...
	public NewMessage getLastMessage(String fromuserid, String touserid) {
		...
		try {
			...
			if (touserid.contains("Group")) {
				...
				query = "select message,fromuserid,messagetime,messagedate from chat where (touserid='" + touserid
						+ "' and fromuserid='" + fromuserid + "') or (fromuserid='" + touserid + "' and touserid='"
						+ fromuserid + "')";
			}
			Statement pr = con.createStatement();
			ResultSet rs = pr.executeQuery(query);
	...
	public int getUndreadMessageCountStudent(Student s) {
		int total = 0;
		String course = s.getCourseCode() + " " + new CourseData().getsemoryear(s.getCourseCode()) + "-"
				+ s.getSemorYear();
		try {
			String query = "select readby from chat where ((touserid='" + course + " Official Group' or touserid='"
					+ course + " Students Group') and fromuserid!='" + s.getUserId() + "') or touserid='"
					+ s.getUserId() + "'";
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery(query);
			while (rs.next()) {
	...
	public int getUndreadMessageCountFaculty(Faculty f) {
		int total = 0;
		String course = f.getCourseCode() + " " + new CourseData().getsemoryear(f.getCourseCode()) + "-"
				+ f.getSemorYear();
		try {
			String query = "select readby from chat where ((touserid='" + course + " Official Group' or touserid='"
					+ course + " Faculties Group') and fromuserid!='" + f.getFacultyId() + "') or touserid='"
					+ f.getFacultyId() + "'";
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery(query);
			while (rs.next()) {
