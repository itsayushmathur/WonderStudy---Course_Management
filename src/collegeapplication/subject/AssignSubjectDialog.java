...
public class AssignSubjectDialog extends JDialog implements ActionListener {
	...
	private AssignSubjectDialog(Faculty f) {
		...
		positioncombo.setModel(new DefaultComboBoxModel<String>(new String[] { "---Select Position---",
		...
		assignsubjectbutton.setBackground(Color.RED);
					...
					if (am != null) {
						if (am.assignsubjectpanel != null && am.assignsubjectpanel.isVisible()) {
							...
							am.viewfacultypanel = new ViewFacultyPanel(
									new FacultyData().getFacultyInfobyId(f.getFacultyId()), am,
